...
func ApplyTransaction(msg *TxMessage, evmEvent *evm.EVM) (executionResult *ExecutionResult) {
	...
	if err := PreCheck(msg, evmEvent); err != nil {
		...
		return NewExecutionResult(0, err, nil, false)
	}
	...
	if GasRemainBefore < ExpenseGasBase {
		...
		return NewExecutionResult(0, errors.New("gas is not enough in ExpenseGas_Base"), nil, false)
	}
	...
	if msg.Value.Sign() > 0 && !evmEvent.Context.CanTransfer(evmEvent.StateDB, msg.From, msg.Value) {
		...
		return NewExecutionResult(0, errors.New("insufficient funds for transfer"), nil, false)
	}

	// 检查初始代码是否超出大小（合约创建）
	if ContractCreation && len(msg.Data) > params.MaxInitCodeSize {
		fmt.Printf("%sERROR MSG%s   数据错误 in 初始代码超出大小\n", types.FRED, types.FRESET)
		return NewExecutionResult(0, errors.New("max init-code size exceeded"), nil, false)
	}
	...
	if EvmError == nil && (IsParallel && !CanParallel) {
		...
		return NewExecutionResult(0, nil, nil, true)
	}
	...
	if EvmError == nil {
		...
		return NewExecutionResult(ExpenseGasBase+GasRemainBefore-GasRemainAfter, nil, ReturnData, false)
	} else {
		...
		return NewExecutionResult(ExpenseGasBase+GasRemainBefore, EvmError, nil, false)
	}
}
...
func NewExecutionResult(UsedGas uint64, Err error, ReturnData []byte, IsParallelError bool) *ExecutionResult {
