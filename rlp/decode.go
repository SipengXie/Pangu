...
// todo DecodeBytes将RLP数据从b解析到val
// 请参阅包级文档以了解解码规则
// 输入必须只包含一个值且没有尾随数据
func DecodeBytes(b []byte, val interface{}) error {
	...
	if err := stream.Decode(val); err != nil {
		return err // !ERROR ERROR ERROR err msg = "too few elements"
	}
	if r.Len() > 0 {
...
func makeStructDecoder(typ reflect.Type) (decoder, error) {
	...
	dec := func(s *Stream, val reflect.Value) (err error) { // ? 为什么会跳到这里 TODO:
		if _, err := s.List(); err != nil {
			return wrapStreamError(err, typ)
		}
		for i, f := range fields {
			err := f.info.decoder(s, val.Field(f.index))
			if err == EOL { // 第四次循环进来了，到了当前列表的末尾 err = error(*errors.errorString) *{s: "rlp: end of list"}
				if f.optional {
				...
				return &decodeError{msg: "too few elements", typ: typ} // !ERROR ERROR ERROR
			} else if err != nil {
...
// todo Decode将解码一个值，并将结果存储在指向val的指针中
// Please see the documentation for the Decode function
// to learn about the decoding rules.
func (s *Stream) Decode(val interface{}) error {
	if val == nil {
		return errDecodeIntoNil // ! rlp: pointer given to Decode must not be nil 传给Decode的指针不能为nil
	}
	rval := reflect.ValueOf(val) // ValueOf返回一个新的Value，其被初始化为存储在接口i中的具体值。ValueOf(nil)返回零值Value
	rtyp := rval.Type()          // Type返回v的类型
	if rtyp.Kind() != reflect.Ptr {
		return errNoPointer // ! rlp: interface given to Decode must be a pointer 传给Decode的接口必须是指针
	}
	if rval.IsNil() {
		return errDecodeIntoNil // ! rlp: pointer given to Decode must not be nil 传给Decode的指针不能为nil
	}
	decoder, err := cachedDecoder(rtyp.Elem()) // 早上看看这个typ是个啥类型，返回的这个decoder应该是个函数
	if err != nil {                            // no error
		return err
	}

	err = decoder(s, rval.Elem()) // ! ERROR ERROR ERROR err 在这里赋值
	if decErr, ok := err.(*decodeError); ok && len(decErr.ctx) > 0 {
		// Add decode target type to error so context has more meaning.
		decErr.ctx = append(decErr.ctx, fmt.Sprint("(", rtyp.Elem(), ")")) // 没走这一步
	}
	return err
}
